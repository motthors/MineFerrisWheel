...
import mfw._mc._1_7_10.message.MessageCommandSendToPlayerStC;
	...
	public void processCommand(ICommandSender sender, String[] strings)
	{
		...
		if(strings.length == 2)
		{
			...
			if(strings[0].equals("sync"))
			{
				...
				{
					player.addChatComponentMessage(new ChatComponentText("/mfw sync [true/false]"));
					return;
				}
			...
			else if(strings[0].equals("renderdist"))
			{
				try{
					...
					player.addChatComponentMessage(new ChatComponentText("/mfw renderdist [1~10]"));
					return;
				}
			...
			{
				player.addChatComponentMessage(new ChatComponentText("/mfw [sync/renderdist] (data)"));
				return;
			}
		...
		else player.addChatComponentMessage(new ChatComponentText("/mfw [sync/renderdist] (data)"));
	}
}